...
def movie_map_data(request):
    ...
    """
    days_window = 30   # last 30 days (increased for better data)
    top_n = 5         # top 5 movies per region
    decay_factor = 0.1  # reduced decay for longer relevance
    ...
    # If no recent items, get all items for demo purposes
    if not items.exists():
        items = Item.objects.all().select_related('movie', 'order')
    ...
    for item in items:
        # Use coordinates if available, otherwise use default region
        if item.order.latitude and item.order.longitude:
            region = f"Region {item.order.latitude:.1f},{item.order.longitude:.1f}"
            coords[region] = {"lat": item.order.latitude, "lng": item.order.longitude}
        else:
            # Default region for orders without coordinates
            region = "Default Region"
            coords[region] = {"lat": 40.7128, "lng": -74.0060}
    ...
    # If no data, create some demo data
    if not scores:
        demo_movies = Movie.objects.all()[:3]
        region = "Demo Region"
        coords[region] = {"lat": 40.7128, "lng": -74.0060}
        for i, movie in enumerate(demo_movies):
            scores[region][movie.name] = 5.0 - i  # Demo scores
